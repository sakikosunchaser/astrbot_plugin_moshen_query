...
@register("astrbot_plugin_welcome", "Sunchser", "一个简单的入群欢迎插件", "1.0.2")
class WelcomePlugin(Star):
    ...
    def _load_config_from_context(self) -> Dict[str, Any]:
        """
        兼容不同 AstrBot 版本的配置注入方式。
        如果 __init__ 没直接拿到 config，则尝试从 context / self 上读取。
        """
        # 方式1：self.config 已存在
        ...
        # 方式4：context.data.plugin_config
